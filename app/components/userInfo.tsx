...
import { reqLogin, reqSign, reqSelf } from "@/app/api/requestApi/user";
import type { loginType, selfType } from "@/app/api/requestApi/user";
...
export function UserInfoMessageModal(props: { onClose: () => void }) {
  return (
    <div className="modal-mask">
      <ModalMini
        title={Locale.UserInfo.Title}
...
export function MessageLogin(props: { onClose: () => void }) {
  ...
  const steps = [
    {
      name: Locale.UserInfo.Steps.Select,
      value: "select",
    },
    {
      name: Locale.UserInfo.Steps.Preview,
      value: "preview",
    },
  ];
    ...
    <>
      ...
      >
        ...
        <div className={stylesLogin["loginBox"]}>
          ...
          {/* <input type="text" placeholder="输入你的账号" value={userForm.username} onChange={(e) => setUserForm({ ...userForm, username: e.target.value })} /> */}
          {/* </div> */}
        ...
        <>
          ...
          {/* <div className={stylesLogin["loginBox"]}>
            ...
            <div className={stylesLogin["inputrow"]}>
              ...
              <input type="text" value={signForm.username} placeholder="起一个名字吧" onChange={(e) => setSignForm({ ...signForm, username: e.target.value })} />

            </div>
            <div className={stylesLogin["label"]}>确认密码</div>
            <div className={stylesLogin["inputrow"]}>
              <PasswordInputWeb type="password" label="密码" placeholder="设置一个密码吧" value={signForm.password} onChange={(e) => setSignForm({ ...signForm, password: e.currentTarget.value })} />
...
export function ImagePreviewer(props: {
  ...
  const copy = () => {
    showToast(Locale.UserInfo.Image.Toast);
...
export function MarkdownPreviewer(props: {
  ...
  const mdText =
    ...
    props.messages
      .map((m) => {
        return m.role === "user"
          ? `## ${Locale.UserInfo.MessageFromYou}:\n${getMessageTextContent(m)}`
          : `## ${Locale.UserInfo.MessageFromChatGPT}:\n${getMessageTextContent(
              m,
            ).trim()}`;
      })
      .join("\n\n");
